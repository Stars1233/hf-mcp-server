...
- `LOG_SKILL_EVENTS`: Enables privacy-preserving historical logging for SEP-2640 Skills probes when `LOGGING_DATASET_ID` and a logging token are configured (default: `true`). Events are written under `skills/YYYY-MM-DD/`, separately from tool queries, and contain only client/protocol correlation, success, duration, cursor presence, and aggregate result counts. Events do not copy requested skill names or URIs, cursor values, frontmatter, resource content, response bodies, or error messages.
